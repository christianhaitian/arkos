...
if [ ! -f "/home/ark/.config/.update09032021" ]; then

	printf "\nRetroArch 1.9.8\n" | tee -a "$LOG_FILE"
	sudo wget --no-check-certificate https://github.com/wummle/arkos/raw/main/09032021/arkosupdate09032021.zip -O /home/ark/arkosupdate09032021.zip -a "$LOG_FILE" || rm -f /home/ark/arkosupdate09032021.zip | tee -a "$LOG_FILE"
	if [ -f "/home/ark/arkosupdate09032021.zip" ]; then
		sudo unzip -X -o /home/ark/arkosupdate09032021.zip -d / | tee -a "$LOG_FILE"
		sudo rm -v /home/ark/arkosupdate09032021.zip | tee -a "$LOG_FILE"
	else 
		printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
		sleep 3
		echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
	...
	touch "/home/ark/.config/.update09032021"
fi

if [ ! -f "/home/ark/.config/.update09042021" ]; then

	printf "\nPSP and EasyRPG bugfix\n" | tee -a "$LOG_FILE"
	sudo wget --no-check-certificate https://github.com/wummle/arkos/raw/main/09042021/arkosupdate09042021.zip -O /home/ark/arkosupdate09042021.zip -a "$LOG_FILE" || rm -f /home/ark/arkosupdate09042021.zip | tee -a "$LOG_FILE"
	if [ -f "/home/ark/arkosupdate09042021.zip" ]; then
		sudo unzip -X -o /home/ark/arkosupdate09042021.zip -d / | tee -a "$LOG_FILE"
		sudo rm -v /home/ark/arkosupdate09042021.zip | tee -a "$LOG_FILE"
	else 
		printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
		sleep 3
		echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
	...
	touch "/home/ark/.config/.update09042021"
fi
if [ ! -f "$UPDATE_DONE" ]; then
    ...
    sudo chmod 777 /etc/emulationstation/es_*.cfg
	sudo chmod 777 /usr/local/bin/am*
	sudo chmod -R 777 /home/ark/.config/retroarch/
	sudo chmod -R 777 /opt/retroarch/
	touch "$UPDATE_DONE"
