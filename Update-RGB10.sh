...
if [ ! -f "/home/ark/.config/.update07282021" ]; then
	...
	if [ -f "/home/ark/arkosupdate07282021.zip" ]; then
		...
		sed -i -e '/<extension>.ctg .CTG<\/extension>/{r /home/ark/fix_ti99.txt' -e 'd}' /etc/emulationstation/es_systems.cfg
		sudo rm -v /home/ark/arkosupdate07282021.zip | tee -a "$LOG_FILE"
		sudo rm -v /home/ark/fix_ti99.txt | tee -a "$LOG_FILE"
	else 
		printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
		sleep 3
		echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
...
if [ ! -f "/home/ark/.config/.update02172023" ]; then
	...
	sudo rm -rf /dev/shm/*
	sudo wget -t 3 -T 60 --no-check-certificate "$LOCATION"/02172023/rgb10/arkosupdate02172023.zip -O /dev/shm/arkosupdate02172023.zip -a "$LOG_FILE" || sudo rm -f /dev/shm/arkosupdate02172023.zip | tee -a "$LOG_FILE"
...
if [ ! -f "/home/ark/.config/.update10162023" ]; then
	...
	if [ -f "/dev/shm/arkosupdate10162023.zip" ]; then
	  if [ -f "/boot/rk3566.dtb" ] || [ -f "/boot/rk3566-OC.dtb" ]; then
        ...
        sudo unzip -X -o /dev/shm/arkosupdate10162023.zip -x opt/system/Wifi.sh opt/system/Advanced/wifion.sh usr/bin/osk usr/bin/msgbox usr/local/bin/watchforbtaudio.sh "opt/inttools/*" usr/local/bin/quickmode.sh usr/local/bin/Enable\ Quick\ Mode.sh usr/local/bin/checknswitchforusbdac.sh usr/local/bin/quickmode.sh usr/local/bin/Enable\ Quick\ Mode.sh -d / | tee -a "$LOG_FILE"
	  fi
	  sudo rm -fv /dev/shm/arkosupdate* | tee -a "$LOG_FILE"
	else
	  printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
...
if [ ! -f "/home/ark/.config/.update08232024" ]; then
	...
	then
	  ...
	  sudo sed -i "/\"5)\") \/opt\/system\/351Files.sh 2>\&1 > \/dev\/tty1/s//\"5)\") \/opt\/system\/351Files.sh 2>\&1 > \/dev\/tty1\n                                ;;\n                          \"6)\") sudo kill -9 \$(pidof boot_controls)\n                                \/opt\/system\/Advanced\/\"Backup ArkOS Settings.sh\" 2>\&1 > \/dev\/tty1\n                                sudo .\/boot_controls none \$param_device \&\n                                ;;\n                          \"7)\") sudo kill -9 \$(pidof boot_controls)\n                                \/opt\/system\/Advanced\/\"Restore ArkOS Settings.sh\" 2>\&1 > \/dev\/tty1\n                                sudo .\/boot_controls none \$param_device \&/" /usr/bin/emulationstation/emulationstation.sh /usr/bin/emulationstation/emulationstation.sh.ra /usr/bin/emulationstation/emulationstation.sh.es
	fi
...
if [ ! -f "/home/ark/.config/.update02082025" ]; then
	...
	if [ -f "/boot/rk3326-r33s-linux.dtb" ] || [ -f "/boot/rk3326-r35s-linux.dtb" ] || [ -f "/boot/rk3326-r36s-linux.dtb" ] || [ -f "/boot/rk3326-rg351v-linux.dtb" ] || [ -f "/boot/rk3326-rg351mp-linux.dtb" ] || [ -f "/boot/rk3326-gameforce-linux.dtb" ]; then
	  ...
	  cp -Rfv /home/ark/filters/filters.64.rk3566/* /home/ark/.config/retroarch/filters/. | tee -a "$LOG_FILE"
	  cp -Rfv /home/ark/filters/filters.32.rk3566/* /home/ark/.config/retroarch32/filters/. | tee -a "$LOG_FILE"
	  rm -rfv /home/ark/filters/ | tee -a "$LOG_FILE"
	  rm -fv /opt/retroarch/bin/retroarch.* | tee -a "$LOG_FILE"
	  rm -fv /opt/retroarch/bin/retroarch32.* | tee -a "$LOG_FILE"
	fi
...
if [ ! -f "/home/ark/.config/.update02092025" ]; then
